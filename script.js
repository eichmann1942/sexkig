...
/* 상태 제어 (로그인 여부 포함) */
let state = JSON.parse(localStorage.getItem("heritageGO_v23")) || {
  tab: "home",
  detailId: null,
  routeId: null,
...
function save() {
  localStorage.setItem("heritageGO_v23", JSON.stringify(state));
}
...
/* 4. 서버 실시간 리스너 (더미 유산 탐험가 제거 로직 포함) */
function initServerListeners() {
  ...
  // 이전 더미 '유산 탐험가' 서버 데이터 정리
  db.ref(`clubs/${currentClubId}/members/user_%EC%9C%A0%EC%82%B0%20%ED%83%90%ED%97%98%EA%B0%80`).remove();
  db.ref(`clubs/${currentClubId}/members/user_유산 탐험가`).remove();

  // 실시간 클럽 멤버 목록 수신 및 더미 필터링
  db.ref(`clubs/${currentClubId}/members`).on('value', (snapshot) => {
    const data = snapshot.val();
    if (data) {
      const rawList = Object.values(data);
      // '유산 탐험가' 더미 계정 필터링 (내가 '유산 탐험가'가 아닌 이상 목록에서 제거)
      state.clubMembers = rawList.filter(m => {
        if (m.id === state.myUserId) return true;
        return m.nickname && m.nickname !== "유산 탐험가";
      });
...
function fullReset() { localStorage.removeItem("heritageGO_v23"); location.reload(); }
